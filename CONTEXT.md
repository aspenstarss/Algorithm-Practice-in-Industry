...
- **页面渲染内核**：纯决策函数（排序/折叠/配色/统计/URL净化）在 `paperBotV2/arxiv_daily/page_logic.py`；前端样式与脚本住 `frontend/`（Python 不内嵌 JS/CSS）；骨架令牌由 `generate_arxiv_html.py` 的 `_apply_tokens` 单点渲染。
